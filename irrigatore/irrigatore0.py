...
maxStaz = 8
stazioni = [4, 17, 18, 22, 23, 24, 25, 27]
...
def sleep(minuti):
    global delay
    if delay: tm.sleep(minuti*5)

def openStazione(stazione, minuti):
    stazione -= 1
...
def exect (tasks, *now):
    ...
    for (ora, programmi) in tasks:
        ...
        if datet > now:
            ...
            sleep(minuti)
            now = dt.datetime.today() if delay else now + dt.timedelta(minutes=minuti)  
        print(ora, "start")
...
def test (n):
    match n:
        case 1:
            ...
            #print(tasks)
            exect(tasks, dt.datetime.combine(dt.datetime.today(), dt.time(8,30)))
        case 2:
            ...
            tasks = [[addm(now, 1), [['a', ((1, 1), (2, 1))], ['b', ((3, 1), (4, 1))]]], [addm(now, 4), [['c', ((1, 1),)]]], [addm(now, 5), [['a', ((1, 1), (2, 1))]]]]
            #print(tasks)
            exect(tasks, now)
    print()
